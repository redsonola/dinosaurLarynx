...
function getWidenessFromPolyRegression(dist : number, z :number, eyeDist: number) : number
{
  // [0.56486529]
...
//include eye distance in training data -- this is the best model so far
  ...
  a[17]*x1*x2*x2 + a[18]*x2*x2*x2 + b;

  Math.max(0, w); //don't allow negative values
  Math.min(1.2, w); //don't allow values greater than 1.2
