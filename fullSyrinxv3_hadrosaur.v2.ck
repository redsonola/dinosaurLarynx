...
//elephant larynx paper
//https://pubmed.ncbi.nlm.nih.gov/24133151/
// how do things scale??

//ostrich syrinx paper, 2003
//https://www.researchgate.net/profile/Hueseyin-Yildiz-13/publication/10694169_A_Study_on_the_Morphological_Structure_of_Syrinx_in_Ostriches_Struthio_camelus/links/60ae003f458515bfb0a35a14/A-Study-on-the-Morphological-Structure-of-Syrinx-in-Ostriches-Struthio-camelus.pdf
//https://link.springer.com/article/10.1186/s12862-019-1544-7
...
class SyrinxMembrane extends Chugen
{
    ...
    //note: Recheck numbers and figures -- this is just the FIRST estimation, refine/correct later -- measurements from 1st trip to Tyrell
    //diameter of trachea is estimated by averaging the adult Corythosaurus hyoid measurements -- 22cm, 20cm, 21cm, ~21cm for EACH hyoid bone
    //Make an assumption -- trachea is somewhat smaller than hyoid apparatus (as it is for crocodiles, birds, and humans)
    //Create the trachea as a circle inside the "triangle" of the hyoid bones, first, equilateral (prob. incorrect, fix)
    //Take the max radius of the trachea as that circle, gives some measurements to work with. (area of triangle)/(semi-perimeter) = inscribed circle radius
    // ~21cm each hyoid bone, then circle inside the triangle has ~6cm radius
    6.0 => float a; //  1/2 diameter of trachea, in cm        ??area of membrane at a point
   
   //for hadrosaur, assume membrane is the same as trachea
    6.0 => float h; //1/2 diameter of the syringeal membrane, 3.5mm, 0.35cm

    //for hadrosaur
    //this cannot really be changed significantly as it assumes a thin membrane for sound production as a general rule
    ...
    //length of trachea from Tom Dudgeon -- speciman in ROM
    113.0 => float L; //length of the trachea, in 7cm -- HOLD for this one
    
        //c => float pM; //material density of the syrinx membrane, from During, et. al, 2017, itself from mammalian not avian folds, kg/m 
   
    //let's go with this density -- can change it around -- there's not research in this aspect and not necessarily indications it would change from hadrosaur
    1 => float pM; //values from Smyth 1000.0 kg/m^3, but it should be g/cm3 to match the rest of the units, so 1 g/cm^3
    
    //okay, we'll have to play with these
...
//NOTE: changing to hadrosaur may make this lumped loss approximation out of the realm of usefulness. 
class WallLossAttenuation extends Chugen
{
    //change to hadrosaur
    113.0 => float L; //in cm --divided by 2 since it is taken at the end of each delay, instead of at the end of the waveguide
    ...
    //change to hadrosaur 
    6.0 => float a; //  1/2 diameter of trachea, in m - NOTE this is from SyrinxMembrane -- 
...
now => time start;
      ...
      "p0 , U , totalX , inP1 , dp0 , mem.dU , d2x[0] , d2x[1] , mem.p1 \n"  => string out;
       fout.write( out ); 
 

 while(now - start < 0.002::second)
 {
...
//--2/21 1p fixed reflection filter
 ...
 function void hadrosaur()
 {
     ...
     0.35 => a; 
     0.35 => h; 
     113.0 => L; //153.7 Hz - resonant tube    
     1.0 => hpOut.gain; 
     10.0 => mem.modT; 
     10.0 => mem.modPG; 
     10.0 => mem2.modT; 
    ...
    while( true )
    {
        ...
        while( hi.recv( msg ) )
        {
            ...
            if( msg.isMouseMotion() )
            {
                ...
                //maxPG for small birds is 60
                60*170.2 => float maxPG;
