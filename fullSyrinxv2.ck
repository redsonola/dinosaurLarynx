...
class SyrinxMembrane extends Chugen
{
    ...
    //pG -  3.0591486389338 --//
    16 => float pG; //pressure from the air sac ==> change to create sound, 0.3 or 300 used in Flectcher 1988, possibly 3 in cm units.
   ...
   // 3.5 => float h; //1/2 diameter of the syringeal membrane, 3.5mm, 0.35cm
    ...
    fun void updateTensionAndW()
    {
        ...
        (dT + diff)*T*10.0 => dT ; //note: this is probably not a good pace, but we'll see??
        curT + dT => curT; 
...
tracheaOut => HPFilter hpOut => dac; //from trachea to sound out
...
(1/(33.5070)) => hpOut.gain; 

//mouseEventLoopControllingAirPressure();

5::second => now; 
now => time start;
 while(now - start < 0.005::second)
 {
