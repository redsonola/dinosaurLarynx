...
class SyrinxMembrane extends Chugen
{
    ...
    0.00118 => float p; //air density from Smyth diss., 0.00118cm, 0.0118 mm ---- NOTE: Needs to match the input -- meters
    ...
    0.35 => float a; //  1/2 diameter of trachea, in 3.5mm        ??area of membrane at a point
    ...
    0.01 => float d; //thickness of the membrane, 100 micrometers, 1mm, 0.01cm
    ...
    //0.1020 => float pM; //material density of the syrinx membrane, from Düring, et. al, 2017, itself from mammalian not avian folds, kg/m 
    ...
    fun void updateU()
    {
        if(totalX > 0)
        {
            ...
            (T/2.0) => float timeStep; 
//            p/(8*a*a*totalX*totalX) => float xFactor; 
  
//            (2*Math.sqrt(a*totalX))/p => D; 
//            timeStep*(dU + D*(p0-p1-(xFactor*U*U))) => dU; //0 < x <= a
    ...
    fun void updateX()
    {
        ...
        for( 0 => int i; i < x.cap() ; i++ )
        {
            ...
            (T/2.0)*(d2x[i] + F/m[i] - 2.0*modifiedK*dx[i] - w[i]*w[i]*(x[i]-x0)) => d2x[i];
...
if( !fout.good() )
{
 ...
 {
       <<< " dp0:" + mem.dp0 +" zG:" + mem.zG + " d2x: " + mem.d2x[0] + "  U: " + mem.U + " F: " + mem.F +  "  x: " + mem.totalX + "  p0:" + mem.p0 + "  p1:" + mem.p1 + "  input p1: " + mem.inP1 >>> ;
