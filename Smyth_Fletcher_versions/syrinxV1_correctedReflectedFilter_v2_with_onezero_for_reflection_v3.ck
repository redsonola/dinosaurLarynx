...
//transfer functions to difference equations
//https://dsp.stackexchange.com/questions/49511/z-domain-transfer-function-to-difference-equation
...
//http://musicweb.ucsd.edu/~trsmyth/wind/First_Order_Shelf.html
//http://musicweb.ucsd.edu/~trsmyth/wind/wind.html
...
//http://musicweb.ucsd.edu/~trsmyth/wind/Digital_Waveguide_Section.html -- TODO: implement mouth as cone
SyrinxMembrane mem => DelayA delay => WallLossAttenuation wa => OneZero lp => blackhole; //from membrane to trachea
lp => HPFilter hpOut => Gain reduce => dac; //from trachea to sound out

lp =>  Flip flip => DelayA delay2 => WallLossAttenuation wa2 => delay; //reflection from trachea end back to bronchus beginning
Gain adder; 
wa2 => adder; 
mem => adder; 
...
while( true )
{
    ...
    while( hi.recv( msg ) )
    {
        ...
        if( msg.isMouseMotion() )
        {
          ...
          msg.scaledCursorX * 400 => float pG; 
          pG => mem.pG;
          <<< "pG: " + mem.pG >>>;
        }
    }
...
//--2/21 1p fixed reflection filter
 function void setParamsForReflectionFilter()
 {
     ...
     (ka*(c/a))/44100.0 => float wT; 
     //1.8775468475739818 => wT ; //this is angular frequency per sample at 44100hz ?? 
     <<< wT >>>; //could be wrong somehow - does not correspond to 13.178 khz???
