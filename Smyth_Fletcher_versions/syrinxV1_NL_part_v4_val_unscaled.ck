...
class SyrinxMembrane extends Chugen
{
    ...
    50.0 => float E; //a number of order 10 to 100 to represent stickiness
   ...
   // 3.5 => float h; //1/2 diameter of the syringeal membrane, 3.5mm, 0.35cm
    ...
    7.0 => float L; //length of the trachea, in 0.7cm -- HOLD for this one
    ...
    100 => float pM; //values from Smyth 1000.0 kg/m^3, but it should be g/cm3 to match the rest of the units, so 1 g/cm^3
    ...
    1.0/SRATE => float T; //to make concurrent with Smyth paper
     ...
     [1.0, 0.8] @=> float epsilonForceCouple[]; //try -- mode 1 should be dominant.
...
now => time start;
 ...
 {
       <<< " dp0:" + mem.dp0 +" d2x[0]:" + mem.d2x[0] + " dU: " + mem.dU + "  U: " + mem.U + " p1: " +  mem.inP1 >>> ;
