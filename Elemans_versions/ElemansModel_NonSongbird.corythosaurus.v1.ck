...
//https://www.researchgate.net/profile/Hanspeter-Herzel/publication/13200728_Modeling_the_role_of_nonhuman_vocal_membranes_in_phonation/links/02e7e532043fe36f27000000/Modeling-the-role-of-nonhuman-vocal-membranes-in-phonation.pdf
//& allosaurus paper of interest: https://palaeo-electronica.org/content/2013/389-allo
//https://anatomypubs.onlinelibrary.wiley.com/doi/full/10.1002/ar.24553
//https://anatomypubs.onlinelibrary.wiley.com/doi/abs/10.1002/ar.25153?casa_token=G86c4yLDR5YAAAAA%3A0H5J8ORgQSt6ABLviHAaA4rUD_u54PNJx27F0BG5ju8x5T3gJ6sqYx_53IUK6yhELry8NrixJWg2z8tp
//https://link.springer.com/book/10.1007/978-3-031-35180-8
//https://www.cell.com/neuron/pdf/S0896-6273(19)30839-6.pdf
//https://www.tandfonline.com/doi/abs/10.1080/09524622.2018.1458249
...
class RingDoveSyrinxLTM extends Chugen
{
    ...
    0.0017 * dinoScale => float m; //mass, table C.1
    ...
    0.1 * Math.sqrt(m*k)  => float r; //damping, table C.1 // 0.0012 => float r; --> moved it back to human, previously 0.001 for dove
...
//they used alligators 31-37cm in length, 0.9 and 1.4kg body mass
    ...
    4.5 => float w;//1/2 trachea width -- Corythosaurus
    2*w => float a;//full trachea width
    21.6 => float l; //length of the trachea, 4.1 table, 0.32 **changed, 8.9-14in. med. 11.45", 31-37" (nose to end of body, not tail) alligator is 1.2cm, 3-4x that of a dove, 18x that for Corythosaurus (est. 6m nose to end of body not tail)?? 24-32.4
    0.003 * dinoScale => float a01; //lower rest area
    0.003 * dinoScale => float a02; //upper rest area, testing a02/a01 = 0.8, testing a convergent syrinx

    //heights
    0.04 * 4 => float d1; //1st mass height -- alligator is: d1=0.25, d2=0.05cm -- perhaps this actually doesn't scale up that much? leave alone for now.
    (0.24 - d1)* 4  => float d2; //2nd mass displacement from first
    (0.28 - (d1+d2))* 4  => float d3; //3rd mass displacement from 2nd
    ...
    -0.03 * dinoScale => float a0min; 
    0.01 * dinoScale => float a0max; 
    ...
    5.0 => float modPs; //a modifier for how quickly dx gets added
...
writer.wavFilename("hadrosaurSoundsV2.wav"); //set file name
null @=> writer; //automatically close file on remove-shred -- is it temporary??



//** Setting filter parameters
116.0 => float L;  //in centimenters
...
1 => int closedMouth; //allow toggle between open and closed mouth modes
